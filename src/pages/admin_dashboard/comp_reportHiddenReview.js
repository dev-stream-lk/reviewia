...
const ReportedPost = (props) => {
  ...
  const { selectedPostId, set, adminBanPost, tabNumber, adminUnBanPost } =
    props;
  ...
  useEffect(async () => {
    if (selectedPostId) {
      let res = await getReviewById(selectedPostId);
      let resPost = await getPostById(res.postId);
      console.log(resPost);
      if (res) {
        setReviewData(res)
        setPostData(resPost);
  ...
  return (
    ...
    >
      <AdminBanPost
        open={openBan}
        setOpen={setOpenBan}
        adminBanPost={adminBanPost}
      />
      <AdminUnBanPost
        open={openUnBan}
        setOpen={setOpenUnBan}
        adminUnBanPost={adminUnBanPost}
      />
      <Controls.Card className={classes.productListcard} variant="outlined">
          ...
          <div className={classes.media}>
            <div style={{ width: 150, height: 150 }}>
              <img
                ...
                src={`${
                  postData.imgURL &&
                  postData.imgURL.length > 0 &&
                  postData.imgURL[0].url
                }`}
              ></img>
            </div>
        ...
        <CardContent>
          ...
          >
            <Controls.Card
              className={classes.productListcard}
              variant="outlined"
            >
              <CardHeader
                style={{
                ...
                subheader={
                  <Typography style={{ color: "white" }}>
                    {getDateTime(reviewData.createdAt)}
                  </Typography>
                }
                avatar={
                  <Avatar
                    style={{ width: 40, height: 40 }}
                    title={`${reviewData.reviewedBy}`}
                    aria-label="recipe"
                  >
                    {reviewData.reviewedBy && reviewData.reviewedBy[0]}
                  </Avatar>
                }
              ></CardHeader>
              {/* <Avatar
      ...
      <Controls.Card className={classes.productListcard} variant="outlined">
        ...
        <CardContent>
          ...
          <Typography
            ...
            style={{ display: "flex", flexDirection: "row" }}
          >
            <Typography style={{ marginRight: 20 }}>Review ID :</Typography>
            {report.subjectId}
          </Typography>
          <Typography
            variant="body2"
...
export default function ReportedHiddenReview() {
  ...
  const getPostReports = async () => {
    setListLoading(true);
    let data = await getReportedReviews();
    if (data) {
          ...
          <Grid container alignItems="center">
            ...
            >
              ...
              <List style={{ height: "75vh", overflow: "auto" }}>
                {selected === 0 ? (
                  reportedPosts.notBanned &&
                  reportedPosts.notBanned.length > 0 ? (
                    reportedPosts.notBanned.map((report, index) => (
                      <Grid
                        key={index}
                        item
                        xs={12}
                  ...
                  reportedPosts.banned.map((report, index) => (
                    <Grid
                      key={index}
                      item
                      xs={12}
          ...
          <Grid container alignItems="center">
            ...
            >
              {loadPost ? (
                ...
                >
                  <Typography>
                    Please select a report for see more details.
                  </Typography>
                </Grid>
              )}
            </Grid>
