...
export const createPost = (data) => {
  ...
  let url =
    HOST + `user/post/create?email=${email}&subcategory=${subCategoryId}`;
  if (brandName) {
    url += `&brand=${brandName}`;
  }
...
export const getAllPostsWithPagin = (
  page = 1,
  size = 10,
  sort = "createdAt",
  order = "asc"
  ...
  return fetch(
    HOST + `public/post?page=${page}&size=${size}&sort=${sort}&order=${order}`,
    requestOptions
  )
    .then(async (res) => {
      if (res.ok) {
...
export const getPostBySearch = (
  ...
  if ("rating" in filters && filters.rating > 0) {
    if (!isEmpty) search_url += ",rate";
    else search_url += "search=rate";
    isEmpty = false;
  ...
  if (
    ...
    if (!isEmpty) search_url += ",category";
    else search_url += "search=category";
  ...
  if (
    ...
    if (!isEmpty) search_url += ",subCategory";
    else search_url += "search=subCategory";
    isEmpty = false;
    search_url += `:${filters.subCategory}`;
  }

  if ("brand" in filters && filters.brand !== "" && filters.brand !== "all") {
    if (!isEmpty) search_url += ",brand";
    else search_url += "search=brand";
    isEmpty = false;
    search_url += `:${filters.brand}`;
  }

  if ("type" in filters && filters.type !== "" && filters.type !== "all") {
    if (!isEmpty) search_url += ",type";
    else search_url += "search=type";
    isEmpty = false;
    search_url += `:${filters.type}`;
  }

  // add paging options
  if (!isEmpty) search_url += "&";
...
export const getRecentPosts = () => {
  ...
  return fetch(
    HOST + "public/posts?page=0&size=8&sort=updatedAt&order=desc",
    requestOptions
  )
    .then(async (res) => {
      if (res.ok) {
...
// Delete post
export const deletePostPermanantly = (postId) => {
  const requestOptions = {
    method: "DELETE",
    headers: {
      "Content-Type": "application/json",
  ...
  return fetch(HOST + `user/post?id=${postId}`, requestOptions)
    .then(async (res) => {
      if (res.ok) {
        return true;
      }
